...
describe('SessionManager state machine', () => {
  ...
  it('start creates folder, sets OBS record dir, returns id+path', async () => {
    const obs = mockObs();
    const sync = vi.fn();
    const mgr = new SessionManager(root, '{date}_{time}_{slug}', obs, sync, log);
  ...
  it('409s on double start', async () => {
    const mgr = new SessionManager(root, '{slug}', mockObs(), vi.fn(), log);
    await mgr.start('a');
  ...
  it('end stops active recording, writes manifest before firing sync, clears session', async () => {
    ...
    const mgr = new SessionManager(root, '{slug}', obs, sync, log);
    await mgr.start('ep1');
    mgr.mark('intro');
  ...
  it('end succeeds even when OBS is down (stop failure never blocks)', async () => {
    ...
    const mgr = new SessionManager(root, '{slug}', obs, vi.fn(), log);
    await mgr.start('ep2');
    const manifest = await mgr.end();
