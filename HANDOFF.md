...
- **amaran lighting Phase 1 LIVE 2026-07-21:** approved OpenAPI secret is stored in macOS Keychain (`ORCHESTRA_AMARAN_OPENAPI` / `orchestra`), never in git/YAML/launchd/logs. `src/clients/amaran.ts` implements the official authenticated local WebSocket protocol with fresh AES-256-GCM request tokens, secret-safe status, serialized requests, and the vendor's 200 ms minimum interval. ORCHESTRA is installed as a running launch agent and authenticated to amaran Desktop. Read-only discovery verified protocol v2 plus four fixtures: `#1 KEY`, `FILL #2`, `amaran Ace 25c LEFT`, `amaran Ace 25c Right`; individual state readback matches the desktop app. New read-only routes: `GET /lighting/status`, `GET /lighting/discover`, `GET /lighting/fixture/:nodeId/state`. `GET /scene/plan` produces a non-executable camera/lighting dry run from validated `sceneRecipe` targets. The `content` profile has a draft 5000K coordinated look (key 45%, fill 20%, Ace pair 10%; A7 IV 1/60, f/4, ISO 500, WB 5000K) and renders 12 commands with zero warnings. No write/executor route exists yet, so this build cannot change a live light or camera. 85/85 tests + typecheck + production build pass. **Next:** Phase 2 Sony read-only enumeration/settings readback; power the A7 IV and close Imaging Edge Remote when requested. Only after both SDKs read back correctly should a separately gated scene executor be added.
...
**Sony Camera Remote SDK — DOWNLOADED + VERIFIED WORKING 2026-07-09.** User registered/downloaded CrSDK v2.02.00 (`~/Downloads/CrSDK_v2/`). Built the reference sample + a minimal test as **native arm64** on this Mac: `SDK::Init()` returns OK, enumerate + release run clean (SDK v2.2.00). Dylibs are **universal (x86_64+arm64) — no Rosetta**; both USB and PTP-IP (Wi-Fi) adapters present. Workspace set up at `native/orchestra-camera/` (CMake + proven `sdktest`; SDK vendored under `vendor/` GITIGNORED — license-restricted/30MB; README documents repopulating it + clearing quarantine with `xattr -dr com.apple.quarantine`). **Next (needs a camera on Wi-Fi/USB to develop+test):** the connect + set-ISO tool — API path confirmed in README (CreateCameraObjectInfoEthernetConnection → Connect → SetDeviceProperty(CrDeviceProperty_IsoSensitivity)). Then wire a per-profile `cameras:` block invoked on /go to lock ISO (the grain fix). Original note follows:
...
**amaran lighting — approved + official protocol verified.** The sanctioned endpoint is the amaran Desktop WebSocket at `ws://127.0.0.1:12345`, protocol v2. Every request carries a fresh ten-second AES-256-GCM token derived from the approved secret; the secret itself is never sent. Official actions include fixture/device/scene discovery plus sleep, intensity, CCT, HSI/RGB, effects, quickshots, and presets. The Phase 1 client and dry-run recipe planner described above are now implemented; live configuration and the first read-only handshake deliberately remain unperformed until credential confirmation.
